...
void loop() {
  ...
  if(digitalRead(PUSH1) == LOW && pushb1 == 0){
        pushb1 = 1;
        ...
        if (Cont == 0){
          ...
          myFile = SD.open("mrmeeseekssd.txt", FILE_READ);
          CargarImagen();
          myFile.close();
        }else if (Cont == 2){
          myFile = SD.open("halloweensd.txt", FILE_READ);
          CargarImagen();
            myFile.close();
        }
      }
   if(digitalRead(PUSH2) == LOW && pushb2 == 0){
        pushb2 = 1;
  ...
  coordenaday = 0;
   int totalBytes = myFile.size();
    if (myFile) {
      ...
      for (seccyc = 0; seccyc < 8; seccyc++){
        for (x = 0; x<20480 ; x++){
          myFile.seek(lastpos);
          while (myFile.available()){
            char caracter = myFile.read();
            if (caracter == ','){
              lastpos = myFile.position();
              break;
            }else if (caracter == '0' && firstz == 0){
              firstz = 1;
            }else if (caracter == 'x'){
        ...
        LCD_Bitmap(0, coordenaday, 320, 32, matriz);
